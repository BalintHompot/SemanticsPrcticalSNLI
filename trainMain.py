...
for encoderClass in [ LSTMEncoder, BiLSTMEncoder, MaxBiLSTMEncoder]:
    ...
    best_params_for_model = paramSweep(encoderClass, data, default_params, param_ranges, metadata, runName="sgd")
    # training model with best params (and saving training plots)
    best_model = construct_and_train_model_with_config(encoderClass, data, best_params_for_model, metadata, runName="sgd")
    # testing the best model
    best_model_results = testModel(best_model, data)
    # saving best model and results
    save_model_and_res(best_model, best_model_results, runName="sgd")
    # running SentEval for the model
    runSentEval(best_model, TEXT, tasks="paper", runName="sgd")
